...
describe("Network", () => {
    describe("getFromAPI", () => {
        it("should be a function", () => {
            const network: Network = new Network("server", [
                { host: "host", port: 1 },
            ]);
        ...
        it("should correctly retrieve data from a Node", async () => {
            ...
            expect(result.data.publicKey).toBe(
                "03364c62f7c5a7948dcaacdc72bac595e8f6e79944e722d05c8346d68aa1331b4a"
            );
        });
    ...
    describe("getDelegatePublicKey", () => {
        ...
        it("should throw an  error for an unknown delegate", async () => {
            ...
            await expect(
                network.getDelegatePublicKey(delegate)
            ).rejects.toThrow();
        });
    });
    ...
    describe("getVoters", () => {
        ...
        it("should throw an  error for an unknown delegate", async () => {
            ...
            const delegatePublicKey: string = "";
            await expect(
                network.getVoters(delegatePublicKey)
            ).rejects.toThrow();
        });
    });
});
