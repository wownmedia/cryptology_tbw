...
class Payouts {
  ...
  generatePayouts (payouts) {
    ...
    for (const [address] of payouts) {
      ...
      logger.info(`Multi payout to ${recipientId} prepared: ${payouts.get(address).div(ARKTOSHI).toFixed(8)}`)
      const amount = this.__convertAmount(payouts.get(address))
      
      totalAmount = totalAmount.plus(amount)
      totalFees = totalFees.plus(FEES)
      transactions.push({address,amount})
    }
    
    const transaction = new Transaction(null, null, vendorField)
    const multiPayment = transaction.createMultiPayment(transactions)

    return {totalAmount, totalFees, multiPayment}
  }
