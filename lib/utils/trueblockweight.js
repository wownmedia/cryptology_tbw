...
class TrueBlockWeight {
  constructor () {
    ...
    this.voters = []
    this.forgedBlocks = []
    this.votersPerForgedBlock = new Map()
    this.voterBalances = []
  ...
  async generatePayouts () {
    try {
      ...
      let {votersPerForgedBlock, voters} = await this.__getVoters()

      this.votersPerForgedBlock = votersPerForgedBlock
      this.voters = voters
  ...
  **/
  ...
  async __getVoters() {
    try {
      let __voters = await this.__getCurrentVoters()
      let voterMutations = await this.__getVoterMutations(__voters)
  ...
  **/
  ...
  __setVotersPerForgedBlock (voterMutations, voters) {
    try {
      let votersRound = this.voters.slice(0)
      ...
      __votersPerForgedBlock.forEach((votersDuringBlock, height) => {
        ...
        let {__voters, __votersRound} = this.__mutateVoters(height, previousHeight, votersRound, voters, __voteTransactions)
        voters = __voters.splice(0)
        votersRound = __votersRound.splice(0)
