...
class TrueBlockWeight {
   ...
   **/
  __findLatestPayouts () {
    this.latestPayouts = new Map(this.delegatePayoutTransactions.map((payout) => [payout.recipientId, payout.height]))
    
    this.latestPayouts.forEach((height, address) => { 
      console.log(`PAID ${address} at height ${height}`)
    })
  }
