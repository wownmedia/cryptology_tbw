...
class TrueBlockWeight {
  ...
  __processBalances () {
    try {
      let votersPerForgedBlock = new Map(this.voterBalances.map(voterBalances => [voterBalances.address, voterBalances.balance]))
      let previousHeight = null
...
console.log(`BALANCE: ${votersPerForgedBlock.get('AJQCNzjvt2zZXYeZ2zpU8g3CVvupVWntXN')} at HEIGHT: ${height}`)
        votersPerForgedBlock = this.__mutateVotersBalances(height, previousHeight, votersPerForgedBlock) 
        previousHeight = height
        this.votersPerForgedBlock.set(height, votersPerForgedBlock)
      })
    } catch (error) {
  ...
  __mutateVotersBalances(height, previousHeight, votersPerForgedBlock) {
    ...
    try {
      ...
      if (transactions.length) {
        for(let item in transactions) {
          ...
          if (votersPerForgedBlock.has(recipientId)) {
            let balance = votersPerForgedBlock.get(recipientId)
            balance = balance.minus(amount)
            if (balance.lt(0)) {
              balance = new BigNumber(0)
            }
            votersPerForgedBlock.set(recipientId, balance)
          }

          if (votersPerForgedBlock.has(senderId)) {
            let balance = votersPerForgedBlock.get(senderId)
            balance = balance.plus(amount)
            balance = balance.plus(fee)
            votersPerForgedBlock.set(senderId, balance)
          }
        }
      }
    
      return votersPerForgedBlock
    } catch (error) {
      logger.error(error)
      return []
    }
