...
class TrueBlockWeight {
  ...
  __setVotersPerForgedBlock() {
    try {
      let votersPerForgedBlock = this.voters.slice(0)
      ...
      this.votersPerForgedBlock.forEach((votersDuringBlock, height) => {
        ...
        votersPerForgedBlock = this.__mutateVoters(height, previousHeight, votersPerForgedBlock) 
        previousHeight = height
        this.votersPerForgedBlock.set(height, votersPerForgedBlock)
        let test = this.votersPerForgedBlock.get(height)
console.log(`HEIGHT: ${height} HAS ${votersPerForgedBlock.length} / ${test.length} VOTERS`)
      })
    } catch (error) {
      logger.error(error)
    }
