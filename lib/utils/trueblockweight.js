...
class TrueBlockWeight {
  ...
  __applyProposal () {
    ...
    for (const [address, balance] of this.payouts) {
      if (this.__isFrequencyMinimumReached) {
        // Percentages
  ...
  __isFrequencyMinimumReached (address) {
    ...
    logger.warn(`Payout to ${address} pending (delay of ${frequency} blocks not yet reached) [${blockMinimums}/${currentBlock}]: ${this.payouts.get(address).div(ARKTOSHI).toFixed(8)}`)
    return false;
  }
}
