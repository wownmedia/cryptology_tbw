  ...
  __applyProposal () {
    ...
    for (const [address, balance] of this.feesPayouts) {
      ...
      if (this.payouts.get(address).lt(this.minPayout)) {
        ...
        const lastBlockHeight = this.forgedBlocks[0].height
        const lastPayoutHeight = this.latestPayouts.get(address)
        const blockMinimums = lastPayoutHeight + frequency;

        // if we have a custom frequency, there's been at least 1 payment, and we are over the mininum frequency threshold
        if (frequency > 0 && lastPayoutHeight > 0 && blockMinimums < lastBlockHeight) {
          logger.warn(`Payout to ${address} pending (delay of ${frequency} blocks not yet reached) [${blockMinimums}/${lastBlockHeight}]: ${this.payouts.get(address).div(ARKTOSHI).toFixed(8)}`)
        } else {
