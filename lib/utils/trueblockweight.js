...
const PAYOUT_SPECIAL_FEES =  process.env.PAYOUT_SPECIAL_FEES ? process.env.PAYOUT_SPECIAL_FEES : true
...
class TrueBlockWeight {
  constructor () {
    ...
    this.share = PAYOUT.lte(1) ? PAYOUT : new BigNumber(0)
    this.feesShare = PAYOUT_FEES.lte(1) ? PAYOUT_FEES : new BigNumber(0)
    this.acfShare = PAYOUT_ACF.lte(1) ? PAYOUT_ACF : new BigNumber(0)
  ...
  __generateShares () {
    ...
    for (let block in this.forgedBlocks) {
      ...
      const totalRewardsThisBlock = new BigNumber(BLOCK_REWARD)
      
      let totalFeesThisBlock = new BigNumber(0)
console.log(`SPECIAL FEES: ${this.forgedBlocks[block].type}; ${PAYOUT_SPECIAL_FEES} `)
      if (this.forgedBlocks[block].type === '0' || PAYOUT_SPECIAL_FEES) {
        totalFeesThisBlock = new BigNumber(this.forgedBlocks[block].fees)
      }
      ...
      for (const [address, balance] of blockVoterBalances) {
        if (this.latestPayouts.get(address) <= this.forgedBlocks[block].height) {
          ...
          let rewardShare = new BigNumber(balance).div(totalVoterBalancesThisBlock).times(totalRewardsThisBlock)
          let pendingPayout = this.payouts.get(address).plus(rewardShare)
          this.payouts.set(address, pendingPayout)

          // Process fee rewards
          let feeShare = new BigNumber(balance).div(totalVoterBalancesThisBlock).times(totalFeesThisBlock)
  ...
  __applyProposal () {
    ...
    const totalFees = new BigNumber(FEES).times(this.payouts.size + this.__getAdminFeeCount() + this.__getACFFeeCount())
    logger.info('==================================================================================')
