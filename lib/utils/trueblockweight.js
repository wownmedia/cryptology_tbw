...
class TrueBlockWeight {
  ...
  __getFrequencyAddress (address) {
    if (CUSTOM_FREQUENCY.hasOwnPropery(address) === true) {
  ...
  __applyProposal () {
    ...
    for (const [address, balance] of this.feesPayouts) {
      ...
      if (this.payouts.get(address).lt(this.minPayout)) {
        ...
        const frequency = __getFrequencyAddress(address)
        const lastBlockHeight = this.forgedBlocks[0].height
        const lastPayoutHeight = this.latestPayouts.get(address)

        if ((lastPayoutHeight + frequency) < lastBlockHeight) {
          logger.warn(`Payout to ${address} pending (delay of ${frequency} blocks not yet reached): ${this.payouts.get(address).div(ARKTOSHI).toFixed(8)}`)
        } else {
