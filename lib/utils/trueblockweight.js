...
class TrueBlockWeight {
  ...
  __generateShares () {
    ...
    forgedBlockBalances.forEach((balance, height) => {
      ...
      let totalBalanceThisBlock = new BigNumber(0)
      const validBalancesThisBlock = new Map(votersThisBlock.map((voter) => {      
        ...
        totalBalanceThisBlock.plus(balance)
        return [voter, balance]
      }))
      
      console.log(`TOTAL BALANCE AT ${height} IS ${totalBalanceThisBlock}`)
    })
