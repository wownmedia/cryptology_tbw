    ...
    try {
      ...
      for (let item in forgedBlocks) {
        ...
        for (let item in validVoters) {
          ...
          const poolHopper = POOL_HOPPING_PROTECTION ? this.currentVoters.indexOf(address) >= 0 : true
console.log(`POOL_HOPPING_PROTECTION ${POOL_HOPPING_PROTECTION}: ${address} = ${this.currentVoters.indexOf(address)} `)
          if (poolHopper && (typeof latestPayout === 'undefined' || latestPayout <= timestamp)) {
