  ...
  async __getVoterSinceHeight () {
    try {
      ...
      const voteTransactions = result.rows.map(tx => {
        ...
        const buffer = Buffer.from(tx.serialized, 'hex')
...
//const data = new Transaction(model.serialized.toString('hex'))
       // const data = Transaction.fromBytes(Buffer.from(tx.serialized, 'hex'))
        logger.info(`VOTER ${JSON.stringify(data)}`) //JSON.stringify(Buffer.from(tx.serialized, 'hex').toJSON())}`)
        return {
