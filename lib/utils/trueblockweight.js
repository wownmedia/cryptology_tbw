  ...
  async __getVoterBalances () {
    try {
      ...
      for (let item in this.voterBalances) {
        ...
        const balance = this.voterBalances[item].balance
logger.warn(`BALANCE: ${balance.toString()}`)        
        if (BLOCKLIST.indexOf(address) >= 0) {
          ...
          logger.warn(`Voter without balance: ${address} removed from payout pool.`)
          this.voterBalances.splice(item, 1)
        }
      }
    ...
    for (let block in this.forgedBlocks) {
      ...
      for (let item in this.transactions) {
        ...
        if (transactionHeight >= blockHeight && transactionHeight < nextBlockHeight) {
          ...
          const amount = new BigNumber(this.transactions[item].amount)
          const fee = new BigNumber(this.transactions[item].fee)

          if (this.voters.indexOf(recipientId) >= 0) {
            let balance = new BigNumber(this.forgedBlocks[block].voterBalances.get(recipientId))
          ...
          if (this.voters.indexOf(senderId) >= 0) {
            let balance = new BigNumber(this.forgedBlocks[block].voterBalances.get(senderId))
  ...
  __generateShares () {
    ...
    for (let block in this.forgedBlocks) {
      ...
      const totalFeesThisBlock = new BigNumber(this.forgedBlocks[block].fees)
