  ...
  __generateShares () {
    ...
    forgedBlockBalances.forEach((votersBalances, height) => {
      ...
      const validBalancesThisBlock = new Map(votersThisBlock.map((address) => {
        ...
        console.log(`VOTER ${address} HAS ${new BigNumber(balance).div(ARKTOSHI).toFixed()} AT ${height}`)
        //if (!balance || balance < MIN_BALANCE) {
        //  balance = 0
        //}
        
        votersBalances = votersBalances.plus(balance)
        return [address, balance]    
      }))
      
      this.votersBalancePerForgedBlock.set(height, null)
      
      console.log(`TOTAL BALANCE AT ${height} IS ${votersBalances.toFixed()}`)
    })
