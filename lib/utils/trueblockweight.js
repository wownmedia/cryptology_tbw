...
class TrueBlockWeight {
  ...
  async generatePayouts () {
    try {
      ...
      const votingDelegateBlocks = await this.__getVotingDelegateBlocks(voters, voterWallets)
  ...
  **/
  ...
  async __getVotingDelegateBlocks (voterWallets) {
    try {
      ...
      logger.info(`WALLETS: ${wallets.length} :: ${voterWallets.length} :: ${JSON.stringify(voterWallets[0])}`)
      ...
      for(let item in result.rows) {
        logger.info(`DELEGATE BLOCK: ${JSON.stringify(result.rows[item])}`)
        if(result.rows[item].hasOwnProperty('generator_public_key') && wallets.indexOf(result.rows[item].generator_public_key) >= 0) {
          const address = wallets[result.rows[item].generator_public_key].address
          const block = {
            address,
