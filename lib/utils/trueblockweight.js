...
const SMALL_WALLET_BONUS = process.env.SMALL_WALLET_BONUS ? JSON.parse(process.env.SMALL_WALLET_BONUS) : {walletLimit: 0, percentage: 0}
...
const POOL_HOPPING_PROTECTION = process.env.POOL_HOPPING_PROTECTION ? parseInt(process.env.POOL_HOPPING_PROTECTION, 10) > 0 : false
...
class TrueBlockWeight {
  ...
  async generatePayouts () {
    try {
      ...
      const {votersPerForgedBlock, voters, currentVoters, voterWallets} = await this.__getVoters(delegatePublicKey, forgedBlocks)
      const {voterBalances, votersPublicKeys} = await this.__getVoterBalances(voters, voterWallets)
      const votingDelegateBlocks = await this.__getVotingDelegateBlocks(voterWallets)
      ...
      const {votersBalancePerForgedBlock, smallWallets} = this.__processBalances(forgedBlocks, voterBalances, transactions, votingDelegateBlocks)

      this.payouts = new Map()
      this.feesPayouts = new Map()
      this.__generateShares(votersPerForgedBlock, forgedBlocks, latestPayoutsTimeStamp, votersBalancePerForgedBlock, currentVoters)
      this.__applyProposal(currentBlock, latestPayouts, smallWallets)
  ...
  **/
  async __getDelegate (delegate) {
    try {
      const getDelegateEndpoint = `/api/v2/delegates/${delegate}/`
      const delegateAPIResults = await network.getFromAPI(getDelegateEndpoint)

      const delegateAddress = delegateAPIResults.hasOwnProperty('data') && delegateAPIResults.data.hasOwnProperty('address') ? delegateAPIResults.data.address : null
      const delegatePublicKey = delegateAPIResults.hasOwnProperty('data') && delegateAPIResults.data.hasOwnProperty('publicKey') ? delegateAPIResults.data.publicKey : null
  ...
  async __getVotingDelegateBlocks (voterWallets) {
    try {
      const wallets = new Map(voterWallets.map((wallet) => [wallet.publicKey, wallet.address]))
      const getVotingDelegatesQuery = queries.getVotingDelegates(this.startBlockHeight)
      const result = await this.psql.query(getVotingDelegatesQuery)
      if (result.rows.length === 0) {
        return []
      }

      const votingDelegateBlocks = []
      for (let item in result.rows) {
        if (result.rows[item].hasOwnProperty('generator_public_key') && wallets.has(result.rows[item].generator_public_key)) {
          const address = wallets.get(result.rows[item].generator_public_key)
          const block = {
            address,
            'height': parseInt(result.rows[item].height, 10),
            'fees': new BigNumber(result.rows[item].total_fee)
          }
  ...
  async __getVotersFromAPI (delegate) {
    try {
      const getVotersEndpoint = `/api/v2/delegates/${delegate}/voters`
      const params = {
        page: 1,
        limit: 100
      }

      let votersAPIResults
      let voters = []
      do {
        votersAPIResults = await network.getFromAPI(getVotersEndpoint, params)
        if (votersAPIResults.hasOwnProperty('data') && votersAPIResults.data.length > 0) {
          voters = voters.concat(votersAPIResults.data)
        }
        params.page++
      } while (votersAPIResults.hasOwnProperty('data') && votersAPIResults.data.length > 0)
  ...
  async __getVoters (delegatePublicKey, forgedBlocks) {
    try {
      const currentVotersFromAPI = await this.__getVotersFromAPI(DELEGATE)
      const currentVoters = await this.__getCurrentVoters(currentVotersFromAPI)
      let __voters = currentVoters.slice(0)
      ...
      const voterWallets = await this.__addMutatedVoters(voterMutations, currentVotersFromAPI, currentVoters)
  ...
  **/
  async __addMutatedVoters (voterMutations, currentVotersFromAPI, currentVoters) {
    try {
      const __allVotersFromAPI = currentVotersFromAPI.slice(0)
      for (let item in voterMutations) {
        if (voterMutations[item].hasOwnProperty('address') && currentVoters.indexOf(voterMutations[item].address) < 0) {
          const address = voterMutations[item].address
          const getWalletEndpoint = `/api/v2/wallets/${address}/`
          const walletAPIResult = await network.getFromAPI(getWalletEndpoint)
          if (walletAPIResult.hasOwnProperty('data') && walletAPIResult.data.hasOwnProperty('address')) {
            __allVotersFromAPI.push(walletAPIResult.data)
          }
        }
      }
      return __allVotersFromAPI
    } catch (error) {
      logger.error(error)
      return []
    }
  ...
  **/
  async __getCurrentVoters (currentVotersFromAPI) {
    try {
      if (currentVotersFromAPI.length === 0) {
        return []
      }

      const voters = currentVotersFromAPI.map(voter => voter.address)
      return voters
    } catch (error) {
      logger.error(`Error retrieving current voters from API: ${error}`)
    }
  }
  ...
  **/
  ...
  __filterVoteTransactionsForRound (voteTransactions, height, previousHeight) {
    if (typeof voteTransactions === 'undefined') {
      return []
    }
  ...
  async __getVoterBalances (voters, voterWallets) {
    try {
      let voterBalances = voterWallets.map((row) => {
        return {
          'address': row.address,
  ...
  __processBalances (forgedBlocks, voterBalances, transactions, votingDelegateBlocks) {
    try {
      const smallWalletLimit = new BigNumber(SMALL_WALLET_BONUS.walletLimit).times(ARKTOSHI)
      let smallWallets = new Map(voterBalances.map(voterBalances => [voterBalances.address, true]))
      ...
      votersBalancePerForgedBlock.forEach((votersDuringBlock, height) => {
        ...
        __voters.forEach((balance, address) => {
          if (new BigNumber(balance).gt(smallWalletLimit) && smallWallets.get(address) === true) {
            logger.warn(`${address} removed from small voters (${new BigNumber(balance).div(ARKTOSHI).toNumber()})`)
    ...
    try {
      ...
      if (__votingDelegateBlocks.length) {
        for (let item in __votingDelegateBlocks) {
          ...
          let balance = new BigNumber(votersBalancePerForgedBlock.get(delegateAddress))
  ...
  __filterPoolHoppers (validVoters, currentVoters) {
  ...
  __sumBalances (walletBalances, validVoters) {
  ...
  __generateShares (votersPerForgedBlock, forgedBlocks, latestPayoutsTimeStamp, votersBalancePerForgedBlock, currentVoters) {
    try {
      ...
      for (let item in forgedBlocks) {
        ...
        let balance = new BigNumber(this.__sumBalances(walletBalances, validVoters))
        ...
        if (POOL_HOPPING_PROTECTION) {
          validVoters = this.__filterPoolHoppers(validVoters, currentVoters)
        }
        ...
        for (let item in validVoters) {
          ...
          // const noPoolHopper = POOL_HOPPING_PROTECTION ? currentVoters.indexOf(address) >= 0 : true
  ...
  __applyProposal (currentBlock, latestPayouts, smallWallets) {
    ...
    for (const [address, balance] of this.payouts) {
      ...
      if (this.__isFrequencyMinimumReached(address, currentBlock, latestPayouts)) {
        // Percentages
        const percentage = this.__getSharePercentage(address, smallWallets)
        const acfPayout = new BigNumber(balance).times(this.acfShare)
        const voterPayout = new BigNumber(balance).times(percentage)
        const delegatePayout = new BigNumber(balance).minus(acfPayout).minus(voterPayout)

        this.delegateProfit = this.delegateProfit.plus(delegatePayout)
        this.payouts.set(address, new BigNumber(voterPayout))
        this.acfDonation = this.acfDonation.plus(acfPayout)
        ...
        const payout = new BigNumber(this.payouts.get(address).plus(this.feesPayouts.get(address)))
        this.payouts.set(address, payout)
        if (this.payouts.get(address).lt(this.minPayout) || this.payouts.get(address).eq(0)) {
          if (this.payouts.get(address).gt(0)) {
  ...
  __getSharePercentage (address, smallWallets) {
    ...
    if (smallWallets.get(address) === true) {
      ...
      return new BigNumber(SMALL_WALLET_BONUS.percentage)
    }
    return this.share
  }
