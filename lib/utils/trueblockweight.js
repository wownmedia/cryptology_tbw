...
class TrueBlockWeight {
  ...
  async generatePayouts () {
    try {
      ...
      let {votersPerForgedBlock, voters} = await this.__getVoters(delegatePublicKey, forgedBlocks)
      ...
      this.__findLatestPayouts(delegatePayoutTransactions)
      this.__processBalances(forgedBlocks)
      this.__generateShares(votersPerForgedBlock, forgedBlocks)
      this.__applyProposal(currentBlock)
  ...
  **/
  ...
  __findLatestPayouts (delegatePayoutTransactions) {
    try {
      this.latestPayouts = new Map(delegatePayoutTransactions.map((payoutTransaction) => [payoutTransaction.recipientId, payoutTransaction.height]))
      this.latestPayoutsTimeStamp = new Map(delegatePayoutTransactions.map((payoutTransaction) => [payoutTransaction.recipientId, payoutTransaction.timestamp]))
    } catch (error) {
      logger.error(error)
    }
  }

  __processBalances (forgedBlocks) {
    try {
  ...
  __generateShares (votersPerForgedBlock, forgedBlocks) {
    try {
...
// TODO: OPTIMIZE THIS
      if (this.__isFrequencyMinimumReached(address, currentBlock)) {
        // Percentages
  ...
  */
  __isFrequencyMinimumReached (address, currentBlock) {
