...
class TrueBlockWeight {
  ...
  __processBalances () {
    try {
      ...
      votersBalancePerForgedBlock.forEach((votersDuringBlock, height) => {
        ...
        previousHeight = height
        votersBalancePerForgedBlock.set(height, __voters.slice(0))
      })
  ...
  __generateShares () {
    ...
    forgedBlockBalances.forEach((votersBalances, height) => {
      ...
      for(let item in votersThisBlock) {
        console.log(`${votersThisBlock[item]} - ${voterBalancesThisBlock.get(votersThisBlock[item])}`)
      }
