...
class TrueBlockWeight {
  ...
  async __getVoterSinceHeight () {
    try {
      const getVoterSinceHeightQuery = queries.getVoterSinceHeight(this.votersPublicKeys, this.startBlockHeight)
      ...
      }).filter(transaction => {return transaction.vote.includes(`${this.publicKey}`)})
