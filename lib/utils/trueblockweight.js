...
class TrueBlockWeight {
  ...
  __setVotersPerForgedBlock() {
    this.votersPerForgedBlock = new Map(this.forgedBlocks.map((block) => [block.height, this.voters]))
    this.votersPerForgedBlock.forEach((voters, height) => {
      console.log(`HEIGHT ${height} ${voters}`)  
    })
  }
