  ...
  async __getVotingDelegateBlocks (voterWallets) {
    try {
      ...
      for(let item in voterWallets) {
       logger.info(`${JSON.stringify(voterWallets[item].isDelegate)} === ${voterWallets[item].isDelegate=== true}`)
        if(voterWallets[item].hasOwnProperty('isDelegate') && voterWallets[item].isDelegate === true) {
          logger.info(`${voterWallets[item].publicKey} === ${voterWallets[item].address}`)
          wallets[`${voterWallets[item].publicKey}`] = voterWallets[item].address
        }
      }
      ...
      for(let item in result.rows) {
        if(result.rows[item].hasOwnProperty('generator_public_key') && wallets.indexOf(result.rows[item].generator_public_key) >= 0) {
          const address = wallets[result.rows[item].generator_public_key]
          const block = {
            address,
