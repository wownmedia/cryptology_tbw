  ...
  __generateShares () {
    try {
      ...
      for(let item in this.forgedBlocks) {
        ...
        const totalFeesThisBlock = this.forgedBlocks[height].fees
        for( let item in validVoters) {
          const address = validVoters[item]
          const voterBalance = walletBalances.get(address)
          
          console.log(`VOTER ${address} BALANCE ${voterBalance.div(ARKTOSHI).toFixed()}`)
          
          // CHECK IF NOT ALREADY PAID
          // Process Block rewards
          let rewardShare = voterBalance.div(balance).times(totalRewardsThisBlock)
        ...
        console.log(`FORGED BLOCK ${height} WITH ${validVoters.length} VOTERS AND ${walletBalances.size} WALLETS WITH ${balance.div(ARKTOSHI).toFixed()}`)
      }
