...
class TrueBlockWeight {
  ...
  __getSharePercentage(address) {
    logger.warn(`CUSTOM TEST: ${address}: ${CUSTOM_PAYOUT_LIST.hasOwnProperty(address)} : ${CUSTOM_PAYOUT_LIST[address]}`)
    if (CUSTOM_PAYOUT_LIST.hasOwnProperty(address)) {
      return CUSTOM_PAYOUT_LIST.address
    }
    return this.share
  }
