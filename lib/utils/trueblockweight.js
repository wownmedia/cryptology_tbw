...
class TrueBlockWeight {
  constructor () {
    ...
    this.voterBalances = []
    this.votersPublicKeys = []
    this.delegatePayoutTransactions = []
    this.transactions = []
    this.latestPayouts = new Map()
    this.latestPayoutsTimeStamp = new Map()
    this.votersBalancePerForgedBlock = new Map()
  }

  async generatePayouts () {
    try {
      await this.psql.connect()
      const delegatePublicKey = await this.__getDelegate()
      await this.__getForgedBlocks(delegatePublicKey)
      await this.__getDelegateTransactions(delegatePublicKey)
      
      let {votersPerForgedBlock, voters} = await this.__getVoters(delegatePublicKey)
      
      
      
      await this.__getVoterBalances(voters) 
      await this.__getTransactions(voters)
      voters = null
      
      await this.psql.close()
      this.__findLatestPayouts()
      this.__processBalances()
      this.__generateShares(votersPerForgedBlock)
      this.__applyProposal()

      const payouts = this.payouts
      const delegateProfit = this.delegateProfit
      const acfDonation = this.acfDonation
      const timestamp = this.forgedBlocks[0].timestamp + 1
  ...
  **/
  ...
  async __getVoters(delegatePublicKey) {
    try {
      ...
      let {votersPerForgedBlock, voters} = this.__setVotersPerForgedBlock(voterMutations, __voters)
  ...
  **/
  ...
  __setVotersPerForgedBlock (voterMutations, voters) {
    try {
  ...
  async __getTransactions (voters) {
    try {
      const getgetTransactionsQuery = queries.getTransactions(voters, this.votersPublicKeys, this.startBlockHeight)
  ...
  __findLatestPayouts () {
    try {
      ...
      this.latestPayoutsTimeStamp = new Map(this.delegatePayoutTransactions.map((payoutTransaction) => [payoutTransaction.recipientId, payoutTransaction.timestamp]))
    } catch (error) {
      logger.error(error)
    }
  }
  ...
  __processBalances () {
    try {
      ...
      let previousHeight = null
      const votersBalancePerForgedBlock = new Map(this.forgedBlocks.map((block) => [block.height, null]))
  ...
  __generateShares (votersPerForgedBlock) {
    try {
      ...
      for (let item in this.forgedBlocks) {
        ...
        const timestamp = this.forgedBlocks[item].timestamp
        ...
        for (let item in validVoters) {
          ...
          const latestPayout = this.latestPayoutsTimeStamp.get(payoutAddress)
...
// TODO: OPTIMIZE THIS
    ...
    // for (const [address, balance] of this.feesPayouts) {
      // console.log(`__applyProposal = ${address} ${this.payouts.get(address)}`)
      // if(this.__isFrequencyMinimumReached(address)) {
        // Percentages
