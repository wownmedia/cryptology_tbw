...
class TrueBlockWeight {
  ...
  **/
  ...
  async __getVoters (delegatePublicKey, forgedBlocks) {
    try {
      ...
      const voterWallets = await this.__addMutatedVoters(voterMutations, currentVotersFromAPI, __voters)
  ...
  **/
  async __addMutatedVoters(voterMutations, currentVotersFromAPI, __voters) {
    try {
      for(let item in voterMutations) {
        if(voterMutations[item].hasOwnProperty('address') && __voters.indexOf(voterMutations[item].address) < 0) {
          const address = voterMutations[item].address
          logger.info(`NOT CURRENTLY A VOTER ${address} ${JSON.stringify(voterMutations[item])}`)
        }
      }
    } catch (error) {
      logger.error(error)
