    ...
    try {
      ...
      for(let item in this.forgedBlocks) {
        ...
        walletBalances.forEach((bal, voter) => {
          ...
          if (validVoters.indexOf(voter) && bal.gte(MIN_BALANCE)) {
            balance = balance.plus(bal)
          }
        })
        
        console.log(`FEES THIS BLOCK ${totalFeesThisBlock.div(ARKTOSHI).toFixed()}; BALANCE THIS BLOCK ${balance.div(ARKTOSHI).toFixed()}; VOTERS: ${validVoters.length}`)
        ...
        for( let item in validVoters) {
          ...
          if(voterBalance.gte(MIN_BALANCE)) {
            ...
            const rewardShare = new BigNumber(voterShare.times(BLOCK_REWARD))
          
rewardsThisBlock = new BigNumber(rewardsThisBlock.plus(rewardShare))        
if(address === 'AJQCNzjvt2zZXYeZ2zpU8g3CVvupVWntXN') {
            ...
            if(totalFeesThisBlock.gt(0)) {
              const feeShare = new BigNumber(voterShare.times(totalFeesThisBlock))
              this.feesPayouts.set(address, feeShare)
            }
                    
  console.log(`${address} has ${voterBalance.div(ARKTOSHI).toFixed()} BALANCE. Gets ${rewardShare.div(ARKTOSHI).toFixed()} FROM ${height} / ${balance}. PENDING ${pendingPayout.div(ARKTOSHI).toFixed()}`)
}
          
          }
        }
        console.log(`TOTAL REWARDS FOR THIS BLOCK: ${rewardsThisBlock.div(ARKTOSHI).toFixed()}`)
      }
