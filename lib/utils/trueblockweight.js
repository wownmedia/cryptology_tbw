...
class TrueBlockWeight {
  ...
  __setVotersPerForgedBlock() {
    try {
      let _votersPerForgedBlock = this.voters.slice(0)
      let previousHeight = null
      const votersPerForgedBlock = new Map(this.forgedBlocks.map((block) => [block.height, []]))
      
      votersPerForgedBlock.forEach((votersDuringBlock, height) => {
        if (previousHeight === null) {
          previousHeight = height +1
        }
        _votersPerForgedBlock = this.__mutateVoters(height, previousHeight, _votersPerForgedBlock) 
        previousHeight = height
        votersPerForgedBlock.set(height, _votersPerForgedBlock.slice(0))
      })
      
      votersPerForgedBlock.forEach((votersDuringBlock, height) => {
        console.log(`AT ${height} HAS ${votersDuringBlock.length} VOTERS`)
      })
  ...
  __processBalances () {
    try {
      let votersBalancePerForgedBlock = new Map(this.voterBalances.map(voterBalances => [voterBalances.address, voterBalances.balance]))
      let previousHeight = null
      this.votersBalancePerForgedBlock = new Map(this.forgedBlocks.map((block) => [block.height, []]))
      
      this.votersBalancePerForgedBlock.forEach((votersDuringBlock, height) => {
        if (previousHeight === null) {
          previousHeight = height +1
        }
        votersBalancePerForgedBlock = this.__mutateVotersBalances(height, previousHeight, votersBalancePerForgedBlock) 
        previousHeight = height
        this.votersBalancePerForgedBlock.set(height, votersBalancePerForgedBlock)
      })
    } catch (error) {
      logger.error(error)
    }
  ...
  __generateShares () {
    ...
    for(let item in this.forgedBlocks) {
      ...
      console.log(`${height} - ${votersThisBlock.length}`)
    }
    
    //const forgedBlockBalances = new Map(this.forgedBlocks.map((block) => [block.height, new BigNumber(0)]))
    //forgedBlockBalances.forEach((votersBalances, height) => {
      ...
      //console.log(`${height} HAS ${votersThisBlock.length}`)
      //votersThisBlock.forEach((address, index) => {
        //console.log(`${height} A ${address} I ${index}`)
      //})
      
      //const validBalancesThisBlock = new Map(votersThisBlock.map((voter) => {      
      //  const balance = voterBalancesThisBlock.get(voter)
      //  console.log(`VOTER: ${voter} ${balance}`)  
      //  votersBalances.plus(balance)
      //  return [voter, balance]
      //}))
      
      //console.log(`TOTAL BALANCE AT ${height} IS ${votersBalances}`)
    //})
