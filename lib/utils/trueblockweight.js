...
class TrueBlockWeight {
  ...
  __setVotersPerForgedBlock() {
    try {
      ...
      this.votersPerForgedBlock = new Map(this.forgedBlocks.map((block) => [block.height, []]))
      
      this.votersPerForgedBlock.forEach((votersDuringBlock, height) => {
        if (previousHeight === null) {
          previousHeight = height +1
        }
        _votersPerForgedBlock = this.__mutateVoters(height, previousHeight, _votersPerForgedBlock) 
        previousHeight = height
        this.votersPerForgedBlock.set(height, _votersPerForgedBlock)
      })
      
      this.votersPerForgedBlock.forEach((votersDuringBlock, height) => {
        console.log(`${height} ${votersDuringBlock.length}`)
      })
