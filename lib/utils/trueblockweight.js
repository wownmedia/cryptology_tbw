  ...
  **/
  ...
  async __getVoters (delegatePublicKey, forgedBlocks) {
    try {
      ...
      logger.info(`VOTERS: ${currentVotersFromAPI.length} / ${voterWallets.length}`)
  ...
  async __addMutatedVoters(voterMutations, currentVotersFromAPI, currentVoters) {
    try {
      ...
      for(let item in voterMutations) {
        logger.info(`mutation ${JSON.stringify(voterMutations[item])}`)
        //if(voterMutations[item].hasOwnProperty('address')) {//} && currentVoters.indexOf(voterMutations[item].address) < 0) {
          ...
          //logger.info(`NOT CURRENTLY A VOTER ${address}`)
          ...
          //if (walletAPIResult.hasOwnProperty('data') && walletAPIResult.data.hasOwnProperty('address')) {
            //currentVoters.push(walletAPIResult.data)
          //}
      }
