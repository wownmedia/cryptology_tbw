...
class TrueBlockWeight {
  ...
  **/
  ...
  async __getVotingDelegateBlocks (voterWallets) {
    try {
      let wallets = new Map(voterWallets.map((wallet) => [wallet.publicKey, wallet.address]))
      for(let item in voterWallets) {
       logger.info(`${JSON.stringify(voterWallets[item].isDelegate)} === ${voterWallets[item].isDelegate=== true}`)
        if(voterWallets[item].hasOwnProperty('isDelegate') && voterWallets[item].isDelegate === true) {
          logger.info(`${voterWallets[item].publicKey} === ${voterWallets[item].address} === ${wallets.get(voterWallets[item].publicKey)}`)
        }
      }
      //let wallets = voterWallets.map(wallet => wallet.publicKey)
      //logger.info(`WALLETS: ${wallets.length} :: ${voterWallets.length} :: ${JSON.stringify(wallets[0])}`)
      ...
      for(let item in result.rows) {
        if(result.rows[item].hasOwnProperty('generator_public_key')) {
