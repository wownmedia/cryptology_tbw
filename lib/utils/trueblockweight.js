...
class TrueBlockWeight {
  ...
  async generatePayouts () {
    try {
      ...
      const payouts = null //this.payouts
      const delegateProfit = null //this.delegateProfit
      const acfDonation = null //this.acfDonation
    ...
    try {
      ...
      for(let item in this.forgedBlocks) {
        ...
        for( let item in validVoters) {
          ...
          let rewardShare = voterShare.times(BLOCK_REWARD)
