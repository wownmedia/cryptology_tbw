...
class TrueBlockWeight {
  ...
  async __getCurrentVoters () {
    try {
      ...
      logger.info(`VOTERS ${this.voters.length} entry ${this.voters[0]}`)
  ...
  async __getVoterMutations () {
    try {
      ...
      // Add voters to this.voters in case they are not already there
      //for(let item in this.voteTransactions) {
        //if (this.voters.indexOf(this.voteTransactions[item].address) < 0) {
          //this.voters.push(this.voteTransactions[item].address)
        //}
      //}
      
      logger.info(`MUTATIONS: ${this.voteTransactions.length}`)
      logger.info(`VOTERS ${this.voters.length} entry ${this.voters[this.voters.length-1]}`)
    } catch (error) {
  ...
  __setVotersPerForgedBlock() {
    ...
    this.votersPerForgedBlock.forEach((votersDuringBlock, height) => {
      ...
      voters = this.__mutateVoters(height, previousHeight, voters) 
      previousHeight = height
      this.votersPerForgedBlock.set(height, voters)
      //console.log(`HEIGHT ${height} ${voters.lenght}`)  
    })
  }
  
  __mutateVoters(height, previousHeight, voters) {
    ...
    if (voteTransactions.length) {
      for(let item in voteTransactions) {
        if(voteTransactions[item].vote.includes('+')) {
          const index = voters.indexOf(voteTransactions[item].address)
          voters.splice(index, 1)
        } else if(voteTransactions[item].vote.includes('-')) {
          voters.push(voteTransactions[item].address)
        }
        //console.log(JSON.stringify(voteTransactions[item]))
      }
      console.log(`MUTATIONS: ${voteTransactions.length} ${height} ${previousHeight} ${voters.length}`)
    }
  ...
  async __getVoterBalances () {
    try {
      ...
      const getVoterBalancesQuery = queries.getVoterBalances(this.publicKey, MIN_BALANCE)
  ...
  async __getVoterSinceHeight () {
    try {
      ...
      for (let item in this.voteTransactions) {
        if (this.voteTransactions[item].hasOwnProperty('vote')) {
          const vote = this.voteTransactions[item].vote
          
//if (vote.includes(`-${this.publicKey}`)) {
  logger.error(`VOTE FOUND ${vote} at ${this.voteTransactions[item].height}`)
//}

          if (vote.includes(`+${this.publicKey}`)) {
            const address = this.voteTransactions[item].address
            const height = this.voteTransactions[item].height
