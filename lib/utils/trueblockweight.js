...
console.log(`FORGED BLOCKS BY VOTERS: ${result.rows.length}`)      
      const votingDelegateBlocks = new Map(result.rows.map( (block) => {
        const height = parseInt(block.height, 10) 
        const address = block.address
        const fee = new BigNumber(block.total_fee)
console.log(`VOTING DELEGATE: ${address} - ${height} - ${fee}`)        
        return [height, new Map([address, fee])]
      }))
  ...
  **/
  ...
  __processBalances (forgedBlocks, voterBalances, transactions, forgingRewards) {
    try {
