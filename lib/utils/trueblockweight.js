...
logger.info(`BALANCES: ${JSON.stringify(row)}`)
        return {
          'address': row.address,
...
logger.info(`BLOCK: ${blockHeight} from ${recipientId} to ${senderId}: ${amount.toFixed(0)} + ${fee.toFixed(0)}`)
        ...
        logger.info(`VOTERBALANCE: ${recipientId} : ${this.forgedBlocks[block].voterBalances.get(recipientId)}`)  
        }
      }
...
//logger.info(`BLOCK: ${block}: ${totalVoterBalancesThisBlock} | FEE: ${totalFeesThisBlock}`)
...
//logger.info(`SHARE: ${address} = ${pendingPayout}`)
