...
console.log(`FORGED BLOCKS BY VOTERS: ${result.rows.length}`)      
      const votingDelegateBlocks = result.rows.map( (block) => {
        ...
        const fees = new BigNumber(block.total_fee)
console.log(`VOTING DELEGATE: ${address} - ${height} - ${fees}`)        
        return {height, address, fees}
      })
...
if (address === 'Aa74QyqAFBsevReox3rMWy6FhMUyJVGPop') {
  const voterBalance = new BigNumber(walletBalances.get(address))
  console.log(`cryptology at ${height} ${voterBalance.div(ARKTOSHI).toFixed()}`) 
}
