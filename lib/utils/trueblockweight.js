...
//vvoters++
        ...
        for( let item in validVoters) {
          ...
          // TODO: CHECK IS ADDRESS WASNT PAYED OUT ALREADY ON REDIRECTED ADDRESS
          const payoutAddress = this.__getRedirectAddress(address)
          const latestPayout = this.latestPayouts.get(payoutAddress)
          if(latestPayout > height) {
//console.log(`LATEST ${address} at ${latestPayout}`)            
            break
          }
...
//console.log(`PF: ${pendingFeesPayout}; PP: ${pendingPayout}`)              
