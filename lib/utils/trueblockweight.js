...
class TrueBlockWeight {
  ...
  async generatePayouts () {
    try {
      ...
      const {votersPerForgedBlock, voters, currentVoters} = await this.__getVoters(delegatePublicKey, forgedBlocks)
  ...
  **/
  ...
  async __getVotersFromAPI(delegate) {
    try {
      ...
      do {
        ...
        logger.info(`VotersAPI: ${params.page} ${voters.length} ${JSON.stringify(voters[voters.length-1])}`)
        params.page++
  ...
  async __getVoters (delegatePublicKey, forgedBlocks) {
    try {
      const currentVoters = await this.__getCurrentVoters(delegatePublicKey)
