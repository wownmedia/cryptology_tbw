  ...
  __generateShares () {
    try {
      ...
      for(let item in this.forgedBlocks) {
        ...
        const walletBalances = this.votersBalancePerForgedBlock.get(height).entries()
        console.log(`FORGED BLOCK ${height} WITH ${validVoters.length} VOTERS AND ${walletBalances.length} WALLETS`)
      }
