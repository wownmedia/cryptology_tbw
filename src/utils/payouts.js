...
const ADMIN_PAYOUT_LIST = process.env.ADMIN_PAYOUT_LIST
  ...
  : {};
const VENDORFIELD_ADMINISTRATIVE_MESSAGE = process.env
  .VENDORFIELD_ADMINISTRATIVE_MESSAGE
  ? process.env.VENDORFIELD_ADMINISTRATIVE_MESSAGE
  : "Administrative Payout";
const VENDORFIELD_MESSAGE = process.env.VENDORFIELD_MESSAGE
  ? process.env.VENDORFIELD_MESSAGE
  : "Voter Share";
const VENDORFIELD_ACF_MESSAGE = process.env.VENDORFIELD_ACF_MESSAGE
  ? process.env.VENDORFIELD_ACF_MESSAGE
  : "ACF Donation";
const ACF = process.env.ACF
  ? process.env.ACF
  : "AWkBFnqvCF4jhqPSdE2HBPJiwaf67tgfGR";
const CUSTOM_REDIRECTIONS = process.env.CUSTOM_REDIRECTIONS
...
class Payouts {
  constructor() {
    if (process.env.NODE_ENV === "test") {
      const testAdmins =
        '{"AdWmUu1qHhFimn78KNLPfZhNKAkyD6mtH5":0.4, "Ab4ZDNapj3wPpKGs5vvubrZoMpJWbzFFMJ":0.6}';
  ...
  generatePayouts(payouts, timestamp = null) {
    ...
    for (const [address] of payouts) {
      ...
      logger.info(
        `Payout to ${recipientId} prepared: ${payouts
          .get(address)
          .div(ARKTOSHI)
          .toFixed(8)}`
      );
  ...
  __getRedirectAddress(address) {
    if (CUSTOM_REDIRECTIONS.hasOwnProperty(address) === true) {
      logger.info(
        `Redirection found for ${address}: ${CUSTOM_REDIRECTIONS[address]}`
      );
  ...
  generateAdminPayouts(totalAmount, timestamp = null) {
    ...
    for (let admin in this.admins) {
      ...
      const customVendorField = this.admins[admin].vendorField
        ? this.admins[admin].vendorField
        : VENDORFIELD_ADMINISTRATIVE_MESSAGE;
  ...
  generateAcfPayout(amount, timestamp = null) {
    amount = new BigNumber(amount);
    logger.info(
      `Donation Payout to ACF prepared: ${amount.div(ARKTOSHI).toFixed(8)}`
    );
    amount = this.__convertAmount(amount);
    const vendorField = `${DELEGATE} - ${VENDORFIELD_ACF_MESSAGE}`;
    const transaction = new Transaction(ACF, amount, vendorField, timestamp);

    return transaction.createTransaction();
  }

  __convertAmount(amount) {
    return parseInt(amount.toFixed(0), 10);
  }
}
